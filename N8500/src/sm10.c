...
static volatile CallBack RelayRefresh = { RefreshRelay, 400, 0 };
...
// @@@@@@@@@@@@@@@  Declaration Global variable  @@@@@@@@@@@@@@
 ...
 P_INTERNAL_DATA volatile BYTE      GenCount1;
 P_INTERNAL_DATA volatile BYTE      GenCount2;
 P_INTERNAL_DATA volatile BYTE      GenCount3;
 P_INTERNAL_DATA volatile BYTE      GenCount4;
 P_INTERNAL_DATA volatile BYTE      GenCount5;
 EXTERN_DATA  BYTE      SFireCount;
 // new added for remote panel
// EXTERN_DATA Status  StoreProcState;
// EXTERN_DATA Status  StoreCallStationState;
// EXTERN_DATA BYTE    RemoteExGroupBuff[2];
// EXTERN_DATA BYTE    RemotePanelCounter;
...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
 ...
 //-----------------------------Delay 100 msec-------------------------------------------------------------------------------------
      // EntryBGMPlay();		//�q���ƾ�All Active_Led ON !!!
      AllCallZone();
      IsAllCall = 0;
    }
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
/********** by andy **********/
/* callstation�e��zone_select protocol��,�A�e�@��[0x15,0x00,0x02,0x00,0x11]
   ��protocol���D��,��K�D��GEC8500�P�_�����}�Үɾ� */
...
// Still Fire
 ...
 //      ResetCallStationBus();
     ...
     if (ProcState == CheckIsAlarmHoldState) {
       IsStartSoftTimer1 = ON;
     }
   }
...
#undef InitCount
 ...
 if (IsFireDiffer()) {
   if (IsAnyFireAction()) {
     ...
     GetTelStatus(&Temp3[0]);
     FireZoneBuffer[0] |= Temp3[1];
     FireZoneBuffer[1] |= Temp3[0];

       // emgency tel
     if ((Temp3[0] != 0x00) || (Temp3[1] != 0x00))   {
       if (!IsPreEmergencyMode) { // Is First Fire
         ClrAllLED();	           // Clear All LED & Relay
         ClrAllRelay();
       }
       GenCount2 = 0;
       ProcState = InitialConfirmFireEmergencyState;
